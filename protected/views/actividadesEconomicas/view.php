<?php
/* @var $this ActividadesEconomicasController */
/* @var $model ActividadesEconomicas */

$this->breadcrumbs=array(
	'Actividades Economicases'=>array('index'),
	$model->id,
);

$this->menu=array(
	array('label'=>'List ActividadesEconomicas', 'url'=>array('index')),
	array('label'=>'Create ActividadesEconomicas', 'url'=>array('create')),
	array('label'=>'Update ActividadesEconomicas', 'url'=>array('update', 'id'=>$model->id)),
	array('label'=>'Delete ActividadesEconomicas', 'url'=>'#', 'linkOptions'=>array('submit'=>array('delete','id'=>$model->id),'confirm'=>'Are you sure you want to delete this item?')),
	array('label'=>'Manage ActividadesEconomicas', 'url'=>array('admin')),
);
?>

<h1>View ActividadesEconomicas #<?php echo $model->id; ?></h1>

<?php $this->widget('zii.widgets.CDetailView', array(
	'data'=>$model,
	'attributes'=>array(
		'id',
		'nombre',
		'orden',
	),
)); ?>
